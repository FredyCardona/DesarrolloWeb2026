/**
 * repositorio.js — Repositorio en memoria de Alumnos
 * Tarea Sesión 7 · Desarrollo Web · UMG
 *
 * Responsabilidad: guardar y recuperar alumnos. NO conoce Express ni HTTP.
 * Esto es el patrón Repository que ya viste en clase: la capa de datos
 * queda aislada de la capa web.
 *
 * Implementación del repositorio en memoria.
 */

/**
 * Datos iniciales para que la API arranque con información.
 * @typedef {{
 *   id: string,
 *   nombre: string,
 *   apellido: string,
 *   email: string,
 *   edad: number
 * }} Alumno
 *
 * @type {Alumno[]}
 */
export const datosSemilla = [
    {
        id: 'a-1',
        nombre: 'Ana',
        apellido: 'López',
        email: 'ana.lopez@umg.edu.gt',
        edad: 20,
    },
    {
        id: 'a-2',
        nombre: 'Luis',
        apellido: 'Pérez',
        email: 'luis.perez@umg.edu.gt',
        edad: 22,
    },
    {
        id: 'a-3',
        nombre: 'Marta',
        apellido: 'García',
        email: 'marta.garcia@umg.edu.gt',
        edad: 21,
    },
];

export class RepositorioAlumnos {
    /**
     * @param {Alumno[]} alumnosIniciales
     */
    constructor(alumnosIniciales = []) {
        this.alumnos = alumnosIniciales.map(
            (alumno) => ({
                ...alumno,
            }),
        );

        this.siguienteId =
            this.alumnos.length + 1;
    }

    /**
     * Devuelve todos los alumnos.
     *
     * Se devuelve una copia para evitar que
     * el arreglo interno sea modificado desde afuera.
     *
     * @returns {Alumno[]}
     */
    listar() {
        return this.alumnos.map(
            (alumno) => ({
                ...alumno,
            }),
        );
    }

    /**
     * Busca un alumno por id.
     *
     * @param {string} id
     * @returns {Alumno | undefined}
     */
    obtener(id) {
        const alumno =
            this.alumnos.find(
                (item) => item.id === id,
            );

        if (!alumno) {
            return undefined;
        }

        return {
            ...alumno,
        };
    }

    /**
     * Crea un alumno nuevo.
     * El id lo genera el repositorio:
     * a-1, a-2, a-3...
     *
     * @param {Omit<Alumno, 'id'>} datos
     * @returns {Alumno}
     */
    crear(datos) {
        const nuevoAlumno = {
            ...datos,
            id: `a-${this.siguienteId++}`,
        };

        this.alumnos.push(
            nuevoAlumno,
        );

        return {
            ...nuevoAlumno,
        };
    }

    /**
     * Actualiza un alumno existente.
     * Solo modifica los campos enviados.
     *
     * @param {string} id
     * @param {Partial<Omit<Alumno, 'id'>>} datos
     * @returns {Alumno | undefined}
     */
    actualizar(id, datos) {
        const indice =
            this.alumnos.findIndex(
                (alumno) =>
                    alumno.id === id,
            );

        if (indice === -1) {
            return undefined;
        }

        const alumnoActualizado = {
            ...this.alumnos[indice],
            ...datos,
            id,
        };

        this.alumnos[indice] =
            alumnoActualizado;

        return {
            ...alumnoActualizado,
        };
    }

    /**
     * Elimina un alumno por id.
     *
     * @param {string} id
     * @returns {boolean}
     */
    eliminar(id) {
        const indice =
            this.alumnos.findIndex(
                (alumno) =>
                    alumno.id === id,
            );

        if (indice === -1) {
            return false;
        }

        this.alumnos.splice(
            indice,
            1,
        );

        return true;
    }
}